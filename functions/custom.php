...
function credits_preview()
{
  ...
  if (!empty(trim($rows['traducator'])))
    echo ("<div class='d-inline-block text-left px-1'><i><i class='fas fa-language'></i> Traducere: " . $rows['traducator'] . "</i> ||</div>");
  if (!empty(trim($rows['verificator'])))
    echo "<div class='d-inline-block text-left px-1'><i><i class='fas fa-edit'></i> Verificare: " . $rows['verificator'] . " </i>||</div>";
  if (!empty(trim($rows['editor'])))
    echo "<div class='d-inline-block text-left px-1'><i><i class='fas fa-keyboard'></i> Typesetting: " . $rows['editor'] . "</i> ||</div>";
  if (!empty(trim($rows['editare_manga'])))
    echo "<div class='d-inline-block text-left px-1'><i><i class='fas fa-keyboard'></i> Editare Manga: " . $rows['editare_manga'] . "</i> ||</div>";
  if (!empty(trim($rows['resincronizare'])))
    echo "<div class='d-inline-block text-left px-1'><i><i class='fas fa-mouse'></i> Resincronizare: " . $rows['resincronizare'] . "</i> ||</div>";
  if (!empty(trim($rows['encoder'])))
    echo "<div class='d-inline-block text-left px-1'><i><i class='fas fa-cogs'></i> Encoding: " . $rows['encoder'] . " </i>||</div>";
  if (!empty(trim($rows['quality-check'])))
    echo "<div class='d-inline-block text-left px-1'><i><i class='fas fa-check-double'></i> Quality Check: " . $rows['quality-check'] . "</i> ||</div>";
}
